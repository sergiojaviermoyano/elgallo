    ...
    <div class="col-xs-12">
      <div class="box">
        <div class="box-header">
          <h3 class="box-title">Marcas de Vehículos</h3>
          <?php
              ...
              <?php
                ...
                foreach($list as $b)
                {
                    ...
                    if (strpos($permission,'Edit') !== false) {
                      echo '<i class="fa fa-fw fa-pencil" style="color: #f39c12; cursor: pointer; margin-left: 15px;" onclick="LoadBrand('.$b['marId'].',\'Edit\')"></i>';
                    }
                    if (strpos($permission,'Del') !== false) {
                      echo '<i class="fa fa-fw fa-times-circle" style="color: #dd4b39; cursor: pointer; margin-left: 15px;" onclick="LoadBrand('.$b['marId'].',\'Del\')"></i>';
                    }
                    if (strpos($permission,'View') !== false) {
                      echo '<i class="fa fa-fw fa-search" style="color: #3c8dbc; cursor: pointer; margin-left: 15px;" onclick="LoadBrand('.$b['marId'].',\'View\')"></i>';
                    }
                    echo '</td>';
                    echo '<td style="text-align: left">'.$b['marDescripcion'].'</td>';
                    echo '</tr>';
              }
              ?>
            </tbody>
  ...
  function LoadBrand(id_, action){
    ...
    WaitingOpen('Cargando Marca');
      $.ajax({
        ...
        url: 'index.php/brand/getBrandVh',
  ...
  $('#btnSave').click(function(){
    ...
    WaitingOpen('Guardando cambios');
      $.ajax({
        ...
        url: 'index.php/brand/setBrandVh',
        success: function(result){
                      WaitingClose();
                      $('#modalBrand').modal('hide');
                      setTimeout("cargarView('brand', 'indexVh', '"+$('#permission').val()+"');",1000);
              },
        error: function(result){
              WaitingClose();
              ProcesarError(result.responseText, 'modalBrand');
